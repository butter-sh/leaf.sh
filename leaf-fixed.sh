...
# Version: 2.0.1 - Fixed JavaScript string escaping
...
generate_docs_page() {
  ...
  # Read README and escape MyST syntax for display
  ...
  # Escape any {{...}} syntax in README so it displays as literal text
  # instead of being removed by MyST.sh as undefined variables
  readme_content=$(escape_myst_for_display "$readme_content")
  
  log_debug "README after escaping (first 200 chars): ${readme_content:0:200}"
  ...
  # Create JSON - use jq to properly escape all values
  local data_file="/tmp/leaf_docs_$.json"
  
  # Create JSON-escaped version of readme for use in JavaScript
  # jq -Rs will properly escape ALL special characters including backticks
  # for safe insertion into JavaScript strings
  local readme_json=$(printf '%s' "$readme_content" | jq -Rs '.')
  
  log_debug "JSON-escaped README (first 200 chars): ${readme_json:0:200}"
  ...
  log_debug "Generated JSON data file: $data_file"
